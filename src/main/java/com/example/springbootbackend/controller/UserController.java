...
//		PersonalDetails personalDetails = personalDetailsRepository.findByUserId(user_id);
	...
	public ResponseEntity<ResumeDetails> updateResumeDetails(@PathVariable Long user_id ,@RequestBody ResumeDetails resumeDetails) {
		...
		List<EducationDetails> educationsDetailsList = educationDetailsRepository.findByUserInfo(userInfo);
		educationDetailsRepository.deleteAll(educationsDetailsList);
		EducationDetails[] educationDetailsArray = new EducationDetails[resumeDetails.getEducationDetails().length];
		...
		List<ExperienceDetails> experienceDetailsList = experienceDetailsRepositoryr.findByUserInfo(userInfo);
		experienceDetailsRepositoryr.deleteAll(experienceDetailsList);
		ExperienceDetails[] experienceDetailsArray = new ExperienceDetails[resumeDetails.getExperienceDetails().length];
		...
		List<SkillsDetails> skillsDetailsList = skillsDetailsRepository.findByUserInfo(userInfo);
		skillsDetailsRepository.deleteAll(skillsDetailsList);
		SkillsDetails[] skillsDetailsArray = new SkillsDetails[resumeDetails.getSkillsDetails().length];
		...
		List<CertificationDetails> certificationDetailsList = certificationDetailsRepository.findByUserInfo(userInfo);
		certificationDetailsRepository.deleteAll(certificationDetailsList);
		CertificationDetails[] certificationDetailsArray = new CertificationDetails[ resumeDetails.getCertificationDetails().length];
